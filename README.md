...
│   │   └── routes/
│   │       ├── __init__.py
│   │       └── users.py      # /users and /users/activate endpoints
│   ├── models/
│   │   ├── __init__.py
│   │   └── schemas.py        # Pydantic models (UserCreate, ActivationRequest)
│   ├── services/
│   │   ├── __init__.py
│   │   ├── user_service.py   # create_user / activate_user logic
│   │   └── email_service.py  # send_activation_email via HTTP third party
│   ├── db/
│   │   ├── __init__.py
│   │   └── connection.py     # PostgreSQL connection + optional pool
│   └── utils/
│       ├── __init__.py
│       └── logging.py        # registration logger + redact_email()
│
├── db-init/
│   └── 01_schema.sql         # CREATE TABLE users + activation_codes
│
├── email-mock/
│   └── server.py             # HTTP mock: POST /send, prints activation code
│
├── tests/
│   ├── test_registration.py  # tests for /users
│   └── test_activation.py    # tests for /users/activate
│
├── docker-compose.yml
├── Dockerfile
├── requirements.txt
...
- Email mock server → http://localhost:3001/send

---

### 2️⃣ Check that the API is running
```bash
...
### 1️⃣ Test User Registration (`POST /users`)
...
### 2️⃣ Test User Activation (POST /users/activate)
